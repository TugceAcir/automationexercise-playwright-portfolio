...
test.describe('Login check API', () => {
  test('@API006 @api @auth @negative login check without an email is refused as a bad request', async ({ api }) => {
  ...
  test('@API007 @api @auth @negative login check for an email that was never registered reports user not found', async ({ api }) => {
