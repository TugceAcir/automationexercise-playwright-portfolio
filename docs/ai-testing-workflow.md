...
The full browser-scenario regression suite runs separately on pushes to `main`, a weekly schedule, and manual dispatch. Live-site workflows are serialized through one concurrency group so automation evidence does not create avoidable load on the public demo site.
...
So the problem is not the assertion, and not the data. It was that a click can produce no submission and the page object accepts that silently: `LoginPage.startSignup()` clicked, then waited only *if* navigation occurred. Four lines below it in the same file, `completeAccountInformation()` already did the right thing — it wraps its click in a helper that confirms the expected request was actually sent.

**Implemented:** `startSignup()` now wraps its click in `actAndConfirmDemoRequest`, which waits for the `POST /signup` and, when that request is not observed, consults a committed-state check before deciding whether to retry — so a submission that never happens is detected and retried instead of silently accepted, and one that merely went unseen is not submitted twice.

**Still open:** *why* Firefox occasionally produces no request on that render. The safeguard detects and recovers from a missing submission regardless of the cause, so the suite is no longer exposed to it — but the underlying browser behaviour is not explained, and this is recorded as unresolved rather than quietly closed.

**The same reasoning later generalised.** `@CART010` was reported flaky in September for a related reason: adding to cart is an asynchronous request, the confirmation modal only renders once it returns, and that one click was never confirmed — so a slow response failed the step and a retry passed it. The same conclusion applied, that a click changing server state has to be confirmed rather than assumed. The guard is now on the confirmed add-to-cart, checkout, payment, account-creation and signup paths; the mutating clicks that remain unguarded are tracked in [AGENTS.md](../AGENTS.md) under Next Work, and are unchanged because none has been observed flaking — classify from evidence before changing them. Two tempting explanations were measured and discarded first: an auto-rotating carousel, which costs real time but was never near a timeout, and a recovery step suspected of abandoning the write, which that code path does not actually reach. Recording the discarded theories matters as much as the fix — both were plausible enough to have been shipped as the answer.
