...
API testing was deliberately held back until the UI suite was operated like a real quality system: protected by CI, readable in business terms, reviewed with intent, and connected to defect triage. It was then added as a separate layer in `api-contract/`, with its own package, workflow and generated counts, so it cannot change the UI evidence. It began read-only, and still is wherever CI runs it unattended. Its read-only suite covers catalog, search, login-check, account-lookup and unsupported-method contracts. A second, manual-only suite covers the account lifecycle - create, update and delete - and is confined to throwaway accounts the package generates for itself, so it never touches an account a person owns; every account it creates is recorded before the request is sent and proven deleted afterwards. The API contract is asserted from the response body, because this API answers HTTP 200 even when it refuses a request. Its scope, commands and current evidence are in [README.md](../README.md) and [AGENTS.md](../AGENTS.md).
...
The project runs five representative automated accessibility scans in Chromium: home, product listing/search results, product detail, a populated cart, and authenticated checkout. Run them separately with `npm run test:a11y`; they are not included in the functional scenario or cross-browser execution totals.
...
This matrix summarizes the distinct risks covered by the UI suite; the current scenario and execution totals are generated into the coverage tables in [README.md](../README.md) and [AGENTS.md](../AGENTS.md). "Positive" groups the successful journeys tagged as smoke or regression; the other columns follow the suite taxonomy above. Detailed scenario IDs and expected results remain in the generated business report and Gherkin export.

| Area | Positive paths | Negative paths | Edge cases | Session and resilience |
| --- | --- | --- | --- | --- |
| Authentication | Register and delete account; logout and login again | Invalid login; required login fields; invalid signup email; duplicate email | Names containing spaces and punctuation | Refresh and browser-back preserve login |
| Cart | Add and remove products; selected quantity; price and line total; cart after login; subscription | Empty cart hides checkout | Remove last item; add the same item twice; recommended item | Refresh, browser-back, and browser-context restart preserve cart |
| Checkout | Registered, register-during-checkout, and login-before-checkout paths; multiple-product review; address match; order placement; invoice | Guest checkout prompt; required payment fields | Long order comment | Refresh, browser-back, and browser-context restart preserve checkout |
| Product discovery | Product list and details; exact search; search-to-detail; brand switching; review submission | No-result search; invalid product route; required and valid review email | Partial search | Refresh and browser-back preserve a healthy product journey |
| Home experience | Core navigation; product discovery sections; subscription | Required and valid subscription email | Plus-address subscription; scroll controls | Refresh and browser-back return a healthy home page |
| Customer support | Contact submission; attachment selection | Required and valid contact email | Long message with punctuation | Browser-back leaves the form usable |
| Categories | Women, men, and kids category browsing | Invalid category route | Nested kids category | Not stateful; session checks are intentionally omitted |
| Navigation | Test Cases and API Testing pages | Not applicable to static links | External tutorial destination | Not stateful; session checks are intentionally omitted |
...
The confidence trend compares **full-regression runs only** — those covering the complete browser-scenario count. Focused runs, such as a single-scenario rerun or a tagged subset, are still recorded in `business-report/history.json` but are excluded from the trend, and the dashboard states how many comparable runs the trend is built from. Without that filter a 1-scenario run would score identically to a complete run and inflate both the best-ever figure and the reliability streak. The trend rows are labelled as the latest and best *full-regression* run rather than as the current run, because the newest full regression may be older than the run being displayed elsewhere on the page.

The reliability streak on the dashboard counts consecutive full-regression runs with **zero failed scenarios**. It is deliberately not labelled a "clean run" count, because Playwright reports a retry-recovered scenario as `flaky` rather than `failed`: such a run has zero failures and does advance the streak, but it is not a fully clean run. The dashboard states that distinction next to the number rather than only here, since a definition placed away from the claim it governs does not reach the reader of the claim. The current run's flaky count is shown separately on the same page, and every run now records that count in `business-report/history.json`. Rows written before the field existed are backfilled when the history is read: `summarizeRun` sorts each browser-scenario execution into exactly one of passed, flaky, failed or skipped, so a stored run's flaky count is the remainder of the other three and is recovered from recorded data rather than invented. A row whose counts predate that partition, or do not add up, keeps no flaky count at all rather than a zero it never proved. The streak itself still counts runs with zero failed scenarios; making it flaky-aware would change a published claim and is a separate decision, not a consequence of storing the number.

The target application is an externally owned public demo site. Run stability can vary with site availability, Cloudflare/bot-protection behavior, origin load, and regional timing. Failures should be classified from artifacts and timestamps before being treated as framework regressions; a retry-recovered run is reported as flaky, not fully clean. `npm run triage:failures` exists to make that classification from evidence rather than from assumption.
...
Cross-browser reporting is intentionally counted as browser-scenario executions: every scenario runs in Chromium, Firefox, and WebKit, so the suite produces three report rows per scenario. The current totals are generated into the coverage tables in README.md and AGENTS.md rather than restated here, so they cannot drift. This makes browser-specific risk visible instead of hiding it behind a single collapsed scenario.
