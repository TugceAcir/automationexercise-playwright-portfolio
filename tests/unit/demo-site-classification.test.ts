...
import { BOT_CHALLENGE_ERROR, TRANSIENT_DEMO_SITE_ERROR, UNCERTAIN_ACTION_OUTCOME_ERROR, classifyFailureCause, isBotChallenge } from '../../shared/demo-site-classification';
...
test('an unprovable action outcome classifies as environment, and is not retryable transient noise', () => {
  const error = new Error(`Deleting the customer account: ${UNCERTAIN_ACTION_OUTCOME_ERROR}`);

  assert.equal(classifyFailureCause(error.message), 'environment');
  assert.equal(isTransientDemoPageError(error), false);
});

test('an action the site kept refusing classifies as environment', () => {
  const message = `Logging out did not take effect after 2 attempts. ${TRANSIENT_DEMO_SITE_ERROR}`;

  assert.equal(classifyFailureCause(message), 'environment');
});
