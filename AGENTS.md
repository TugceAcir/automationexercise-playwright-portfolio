...
- Keep API testing isolated in `api-contract/`. Account-changing endpoints live in the lifecycle suite and touch nothing but accounts the package generated itself. It became automatic on 2026-09-24, after three clean manual dispatches; widening the blast radius further - a fixed account, anyone else's data, or writes from a pull request - needs its own approved plan before any code.
...
- Read *which step* failed before diagnosing a red CI run. A failure at `Preflight public demo site` is the environment, at `Typecheck`/`Lint` it is the code, and at the suite step it is a test - three unrelated problems. If the suite step reads `skipped`, no test ran at all and nothing is broken. The demo site serves a bot-challenge page to runners occasionally; it is IP-based and transient, and a local preflight can pass at the same moment.
...
`business-report/history.json` is the project's only unrecoverable asset: it is gitignored, capped at 30 rows, and appended by **every** Playwright run through the reporter registered in `playwright.config.ts` - not only by `npm run business-report`. Each append evicts the oldest row, and an eviction cannot be recovered from git.

- **Never pass `--reporter` to the full suite.** It *overrides* the configured reporters rather than adding to them, so no `results.json` is written, no history row is appended, and `npm run triage:failures` then silently reads a **stale** file and reports "0 confirmed failures". A run that reports zero failures because it never wrote its results is the worst failure mode this repo has. Run it bare.
- **Never run `npm run business-report` straight after a suite.** It re-reads the same `results.json` and appends a duplicate row, evicting a real one for no new information. The same applies to regenerating it just to refresh a screenshot.
- `npm run test:a11y` is the only suite safe to run locally at will: `playwright.a11y.config.ts` does not register the business reporter, so it cannot touch history. A focused `--grep` run with `--reporter=line` is the one narrow exception to the rule above, because it is not the full suite.
- Keep the machine awake for a full run. A hibernation once killed one mid-flight and appended a junk row that had to be removed by hand.

### Dependency changes

- `@playwright/test` and `@axe-core/playwright` must move **together**. axe-core declares an open peer range on `playwright-core`, so npm resolves it independently and bumping one alone installs two copies, failing typecheck on incompatible `Page` types. Never paper over that with `@ts-ignore`.
- Run `npx playwright install` after any Playwright version change. Browser binaries are version-pinned, and a stale set fails every test in 2-3 ms with "Executable doesn't exist" - that is a launch error, not a broken suite.
- Reset `package-lock.json` from `main` before reinstalling when changing versions; npm does not reliably downgrade transitive packages, and stale resolutions produce confusing duplicate-package errors.
...
- Citations perish at source, which is why they are written out in full here. A run log expires about 90 days after the run, and a published dashboard row rolls off once 30 further runs have published (`HISTORY_LIMIT` in `scripts/business-report/history-recovery.ts`). After that the quoted summary line and dashboard figures in this file are the **only** surviving record of what the run said - a third party can no longer re-check them at source, though the claim itself survives in git history. So never shorten or summarise an existing citation to tidy the file, and when `AGENTS.md` is being edited anyway, re-cite the then-current run after actually reading its log.
...
<!-- api-coverage:end -->
- API contract layer, verified 2026-09-24: the current citation is automatic run [`35983468670`](https://github.com/TugceAcir/automationexercise-playwright-portfolio/actions/runs/35983468670) on `dceb30b`, started by `workflow_run` after the clean full regression cited above. **It is the first run in which both suites executed automatically**, and the first evidence that may be cited for write coverage. Read suite: `12 passed (3.5s)` with no trailing clause, job summary `| 12 | 0 | 0 | 0 |`. Lifecycle suite: `4 passed (4.1s)` bare, job summary `| 4 | 0 | 0 | 0 |`. Both report 0 environment, 0 contract and 0 needs-review failures; the whole log carries no `retry #` and no failure mark. Its cleanup section reads `Run 94f4618b-0750-4f9a-9d68-cec7fe445f44: 4 account(s) attempted, 4 deletion(s) proven, 0 leftover(s)`, and the artifact uploaded at 410,635 bytes.
- Two things about that run were checked in ways a summary line cannot show, because a green run that skipped the write steps would look identical to one that ran them. First, **the lifecycle steps genuinely executed**: step 14, `Run account lifecycle suite`, concluded `success` in the job's step list - that list is the only place which distinguishes "ran and passed" from "never ran". Second, **the cleanup record is provably this run's own**: the summary rendered cleanup totals, and a mismatched record would instead have printed "the cleanup record belongs to run X, not the current run Y", so the strict reader in `src/cleanup-evidence.ts` accepted it.
- Write coverage was claimed only after four runs, not one: three manual dispatches on three separate days - [`35595552318`](https://github.com/TugceAcir/automationexercise-playwright-portfolio/actions/runs/35595552318) (2026-09-21), [`35704264137`](https://github.com/TugceAcir/automationexercise-playwright-portfolio/actions/runs/35704264137) (2026-09-22) and [`35841379144`](https://github.com/TugceAcir/automationexercise-playwright-portfolio/actions/runs/35841379144) (2026-09-23), each `4 passed` bare with its own run ID and zero leftovers, 12 accounts created and all 12 proven deleted - and then the automatic run above. The manual runs prove the suite is safe; the automatic run proves the promotion wiring works. They are different claims and both were required.
- Preceding read-only API citations, for continuity: `35522072185` on `801e17d`, `12 passed (3.7s)` bare, in which only the read job appeared and the manual lifecycle job did not run at all - the read/write split demonstrated in CI before the promotion; and before it `35470019631` on `86baeb5`, `12 passed (3.1s)` bare, the first CI run of the Part A runner and per-suite results path.
...
- Branch protection on `main` is deliberately shaped, and two of its settings look loose but are not. The required check is **`Quality Gate and Smoke Session`**, matched by that exact string - renaming the job silently disables the gate. Never add the full-regression job, or any job that is conditionally skipped, to the required checks: a check that never runs on a healthy pull request deadlocks every pull request permanently. `required_approving_review_count: 0` and `enforce_admins: false` are set because a solo maintainer cannot approve their own pull request and would otherwise be locked out of merging anything. `strict: true` is on, so a branch must be current before merging.
...
- API lifecycle: **done**. The suite is promoted, and both the manual safety bar and the first automatic run are cited in Status above. Nothing is outstanding on it. If it ever fails automatically, that is a loud environment failure by design: classify it, read the cleanup section for leftover emails, recover them with `npm run cleanup:leftovers`, and do not rerun blindly - especially after an uncertain create.
