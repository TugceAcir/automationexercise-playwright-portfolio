...
<!-- coverage:end -->
- Last fully green verified run: 2026-09-09. Full-regression workflow run [`34371515757`](https://github.com/TugceAcir/automationexercise-playwright-portfolio/actions/runs/34371515757) on commit `a916b9f` produced 207/207 passed browser-scenario executions in 39.3m, with no failed, no skipped and no flaky results. The run log's suite summary line reads `207 passed (39.3m)` with no trailing clause, and the accessibility scans read `5 passed (51.6s)`; the published dashboard row for it records `total 207 | passed 207 | failed 0 | skipped 0 | flaky 0 | scope full-regression`. Read that summary line rather than the run's conclusion: a run whose scenarios all recover on retry is reported flaky and still exits 0, so a green tick alone does not establish a clean run. The cited commit is not `main`'s head; a citation names the commit whose run was verified, not the newest commit.
- Preceding clean run, for continuity: `34366572432` on `2e9c4c1`, also 207/207 bare. These are the two full regressions following the `@CART010` fix in #36.
...
- Evidence retention: since #35, Playwright report and raw-result uploads run on `!cancelled()` rather than `failure()`, so a flaky run keeps its trace instead of discarding it. Confirmed on run `34371515757`, where those steps ran on a clean run.
...
- Known hole, accepted rather than closed: `coverage:check` verifies the generated counts in `README.md` and `AGENTS.md` only, so nothing mechanically prevents scenario totals being typed back into `docs/test-strategy.md`. The typed totals there were removed, and that file states its totals live in the generated tables - extending the checker was judged more surface area than the drift justifies.
