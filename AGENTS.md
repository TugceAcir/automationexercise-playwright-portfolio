...
- API lifecycle: the manual safety bar is **met**. Three clean dispatches on three separate days - `35595552318` (2026-09-21), `35704264137` (2026-09-22) and `35841379144` (2026-09-23) - each `4 passed` bare with its own run ID, 12 accounts created and all 12 proven deleted, zero leftovers. The promotion followed: the lifecycle steps now run inside the existing API job on the automatic trigger only.
- API lifecycle, next: **read the first automatic run before claiming anything from it** - both its lifecycle result and its cleanup summary. The three manual runs prove the suite is safe; they do not prove the promotion wiring works, and those are different claims. No verified-run citation and no recruiter-facing claim about write coverage until that run has been read.
