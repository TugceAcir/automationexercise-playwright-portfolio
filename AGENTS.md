...
<!-- coverage:end -->
- Last fully green verified run: 2026-08-11. `npx playwright test` produced 210/210 passed browser-scenario executions in 51.9m with `RETRIES=0`, no skipped and no flaky results, and no scenario requiring more than one attempt. The Playwright JSON report and `.last-run.json` are the verification sources for this run. New failed runs should be recorded with `npm run triage:failures` classification rather than treated as equivalent framework regressions.
